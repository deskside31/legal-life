...
async function fetchRecentInformation(url) {
    ...
    try {
        ...
        if (!sourceElement) {
            ...
            container.innerHTML = '<p class="no-info-msg">データの抽出に失敗しました（ID未設定）</p>';
            return;
        }

        const allItems = Array.from(sourceElement.getElementsByClassName('home_info_item'));
        ...
        if (allItems.length > 0) {
            ...
            container.innerHTML = '<p class="no-info-msg">最新のお知らせはありません</p>';
        }
